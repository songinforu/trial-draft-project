<script setup>
import { getDemandHomeList } from "@/api";
...
onMounted(() => {
  getDemandHomeList().then((res) => {
    mainList.value = res.map((o) => {
      o.imgUrl = IMG_URL + o.avatar;
  ...
  <div class="demand-homeList-container">
    <div class="demand-homeList-center-wrapper">
      ...
      <div class="mt-32px grid grid-cols-3 gap-16px">
        <div class="card-item" v-for="item in mainList" :key="item.id">
          <div class="p-24px">
            <p class="card-item-title">{{ item.title }}</p>
            <p class="card-item-price">¥{{ item.priceFen }}</p>
            ...
            <div
              ...
              class="w-full h-40px mt-15.5px"
            >
              <div
                class="user-wrapper flex items-center justify-between"
                v-if="isHover !== item.id"
              >
                <div class="flex items-center">
                  <img :src="item.imgUrl" alt="" width="32" height="32" />
                  <span class="ml-8px">{{ item.companyName }}</span>
                </div>
                <p>
                  {{ item.publishTimeDetail }}
                  <span class="ml-2px">发布</span>
                </p>
              </div>
              <div class="flex items-center justify-center" v-else>
                <p class="look-detail-wrapper flex items-center justify-center cursor-pointer">查看详情</p>
              </div>
            </div>
          </div>
        </div>
...
.demand-homeList-container {
  height: 832px;
  background: url(@/assets/images/demand-homeList-bg.png) no-repeat center
    center;
