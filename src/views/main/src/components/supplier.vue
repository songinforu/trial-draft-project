...
onMounted(() => {
  getSupplier({ pagesize: 6 }).then((res) => {
    const list = res.content;
    mainList.value = list.map((o) => {
      o.avatarUrl = IMG_URL + o.avatar;
        ...
        >
          <div class="p-24px pt-16px">
            ...
            <div class="footer-wrapper">
              <div class="flex items-center">
                <img :src="item.avatarUrl" alt="" width="32" height="32" />
                <span
                  class="ml-8px "
                  >{{ item.createUserName }}</span
                >
              </div>
              <p class="start-time">{{ DateUtil.getFormat(item.createTime, "yyyy-MM-dd") }}发布</p>
            </div>
